...
class Projectile(pygame.sprite.Sprite):
    ...
    def __init__(self, x: int, y: int, direction: int,
        ...
        # Load and orient the arrow image
        ...
        # Stuck state (when inserted into wall, ground, or sprite)
        self.stuck = False
        ...
        # —— Parent attachment support ——
        # If stuck into a moving sprite, store reference and offsets
        self.parent = None
        self.offset_x = 0
        self.offset_y = 0

    def update(self, dt: float):
        # If attached to a parent sprite, follow its movement exactly
        if self.parent is not None:
            self.rect.x = self.parent.rect.x + self.offset_x
            self.rect.y = self.parent.rect.y + self.offset_y
            return

        # Scale dt for arrow physics
        dt = dt * 0.4

        if not self.stuck:
            # Flight: horizontal motion + gravitational drop
            self.rect.x += int(self.direction * self.speed * dt)
            self.vel_y += self.gravity * dt
            self.rect.y += int(self.vel_y * dt)
        else:
            # Once stuck: only handle lifetime and flashing
            self.ground_timer += dt
            time_left = self.ground_life - self.ground_timer
            if time_left <= self.flash_threshold:
                # Flash when near end of life
